...
import { useCategoriesQuery, useFileUpload, useProductsQuery } from '@/graphql/hooks';
...
export default function BookingProductTab() {
  ...
  const onChangeProducts = (ids: string[]) => {
    const results: number[] = [];
    for (const id of ids) {
      const product = productMap.get(id);

      if (product?.id) results.push(Number(product.id));
    }

    form.setValue('product_ids', results);
  };
    ...
    try {
      ...
      if (response?.createBooking) {
        toast({
          ...
          description: 'Đặt hàng của bạn đã được gửi thành công.',
          variant: 'success',
        });
      }
  ...
  return (
    <TabsContent value="product" className="bg-white rounded-md shadow-sm border p-6">
      <div className="mb-6">
        <h4 className="text-xl font-serif font-medium mb-2">Đặt hàng sản phẩm tùy chỉnh</h4>
        <p className="text-muted-foreground text-sm">
          Đặt hàng sản phẩm thủ công được tùy chỉnh để phù hợp với nhu cầu và sở thích của bạn.
        </p>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  >
                    <MultipleSelectTrigger placeholder="Chọn loại sản phẩm">
                      {field.value?.map((id) => (
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Sản phẩm mẫu</FormLabel>
                  <MultipleSelect
                    value={(field.value || [])?.map((id) => id.toString())}
                    onValueChange={onChangeProducts}
                  >
                    <MultipleSelectTrigger placeholder="Chọn sản phẩm mẫu">
                      {field.value?.map((id) => (
                        <MultipleSelectValue key={id} value={id.toString()}>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Mô tả yêu cầu của bạn</FormLabel>
                <FormControl>
                  <Textarea
                    placeholder="Vui lòng cung cấp thông tin chi tiết về yêu cầu đặt hàng tùy chỉnh của bạn, bao gồm kích thước, màu sắc, chất liệu và bất kỳ yêu cầu cụ thể nào..."
